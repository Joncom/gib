...
.defines(function(){"use strict";
    ...
    var MyGame = ig.Game.extend({
        ...
        draw: function() {
            ...
            if(ig.input.state('SCORE')) {
                ...
                for(var name in this.clients) {
                    this.font.draw(name, 10, y);
                    y += 16;
                }
            }

            var debugInfo = [];
            debugInfo.push('Ping: ' + ig.game.latency + 'ms');
            if(this.player) {
        ...
        handle_snapshot: function(snapshot) {
            ...
            // Update local ping info.
            // FIXME: Use ping in ig.game.clients instead.
            for(var name in snapshot.clients) {
                if(name === ig.game.player_name) {
                    var latency = snapshot.clients[name].latency;
                    ig.game.latency = latency;
                }
            }
