...
.defines(function() {
    ...
    Server = ig.Game.extend({
        ...
        sockets_by_name: {},
        waiting_to_respawn: [], // list of names
            ...
            ig.io.sockets.on('connection', function(socket) {
                ...
                socket.on('disconnect', function() {
                    ...
                    if(socket.name) {
                        ...
                        var list = ig.game.waiting_to_respawn;
                ...
                */
                ...
                socket.on('command', function(command) {
                    ...
                    // Is the player dead?
                    if(ig.game.is_waiting_to_respawn(socket.name)) {
        ...
        draw: function() {
            ...
            for(var i=0; i<this.waiting_to_respawn.length; i++) {
                if(i !== 0) string += ', ';
                string += this.waiting_to_respawn[i];
            }
        ...
        is_waiting_to_respawn: function(name) {
            var list = this.waiting_to_respawn;
            for(var i=0; i<list.length; i++) {
                if(name === list[i]) {
                    return true;
                }
