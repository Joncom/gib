...
.requires(
    ...
    'game.server.entities.railgun-attack',
    'game.server.entities.missile-attack',
    'game.server.entities.upgrade',
...
.defines(function() {
    ...
    Server = ig.Game.extend({
        ...
        init: function() {
            ...
            // Spawn an upgrade
            var pos = this.get_random_spawn_position();
            this.spawnEntity(EntityUpgrade, pos.x, pos.y);
            ...
            ig.io.sockets.on('connection', function(socket) {
                ...
                socket.on('disconnect', function() {
                    ...
                    if(socket.name) {
                        ...
                        if(entity) {
                            ...
                            // Make sure we drop his upgrade if he has one
                            if(entity.upgraded) {
                                ig.game.spawnEntity(EntityUpgrade, entity.pos.x, entity.pos.y);
                            }
                ...
                socket.on('pong', function(uuid) {
                    ig.game.log('Received pong: ' + uuid);
                    ...
                    var latency = Math.floor((timestamp - ping.timestamp) / 2);
        ...
        take_snapshot: function() {
            ...
            var properties = {
                ...
                EntityTracer: ['pos', 'end_point'],
                EntityCorpse: ['pos'],
                EntityExplosion: ['pos', 'animProgress'],
                EntityUpgrade: ['pos'],
                EntityDot: ['pos']
            };
            ...
            for(var type in properties) {
                ...
                for(var i=0; i<entities.length; i++) {
                    ...
                    if(!entity.name) {
                        console.error("Cannot snapshot entity type '" + type + "' without name.");
                        continue;
                    }
