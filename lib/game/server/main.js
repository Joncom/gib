...
.defines(function() {
    ...
    Server = ig.Game.extend({
        ...
        // FIXME: store in ig.game.clients instead...
        sockets_by_name: {},
        ...
        init: function() {
            ...
            ig.io.sockets.on('connection', function(socket) {
                ...
                socket.on('init', function(name) {
                    ...
                    ig.game.clients[name] = ig.copy(ig.CLIENT_TEMPLATE);
        ...
        draw: function() {
            ...
            // Show named socket count.
            var count = 0;
            for(var name in this.sockets_by_name) {
                count++;
            }
            debug_array.push('Named Sockets: ' + count);

            // Show dead player count.
            debug_array.push('Dead: ' + this.dead_list.length);
