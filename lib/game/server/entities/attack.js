...
.defines(function() {
    ...
    EntityAttack.inject({
        ...
        init: function(x, y, settings) {
            ...
            // Find point where attack intersects collision tile.
            var lines = [{
                x1: x,
                y1: y,
                x2: x + m_tilesize,
            ...
            for(var i=0; i<lines.length; i++) {
                var line = lines[i];
                var intersect = get_line_intersect_point(start_x, start_y, end_x, end_y, line.x1, line.y1, line.x2, line.y2 );
                if(get_line_intersect_point) {
                    ig.game.spawnEntity(EntityDot, intersect.x, intersect.y);
                    ig.io.emit('info', JSON.stringify(intersect));
                    break;
                }
            }
