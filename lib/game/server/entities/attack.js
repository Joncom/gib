...
.defines(function() {
    ...
    EntityAttack.inject({
        ...
        init: function(x, y, settings) {
            ...
            var tile_offsets = [ // used to check nearby tiles
                {x: 0, y: 0},  // main
                {x: -1, y: 0}, // left
                {x: 1, y: 0},  // right
                {x: 0, y: -1}, // top
            ...
            for(var j=0; j<tile_offsets.length; j++) {
                var tile_offset = tile_offsets[j];

                var tile = {
                    x: res.hit.x + tile_offset.x,
                    y: res.hit.y + tile_offset.y
                };

                // Is the tile traversable?
                if(
                    ig.game.collisionMap.data[tile.y] === undefined ||
                    ig.game.collisionMap.data[tile.y][tile.x] === undefined ||
                    ig.game.collisionMap.data[tile.y][tile.x] === 0
                ) {
                    continue; // ..then we're not interested.
                }

                for(var i=0; i<lines.length; i++) {
                    var line = lines[i];
                    var intersect = get_line_intersect_point(
                        start_x,
                        start_y,
                        this.end_point.x,
                        this.end_point.y,
                        line.x1 + tile_offset.x * tilesize,
                        line.y1 + tile_offset.y * tilesize,
                        line.x2 + tile_offset.x * tilesize,
                        line.y2 + tile_offset.y * tilesize
                    );
                    if(intersect) {
