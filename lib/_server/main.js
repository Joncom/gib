...
When a state change is received from user:
- update entity
- send the state change to all users within frustum
- send the state change to all zone servers within frustum

When a state change is received from zone server:
- send the state change to all users within frustum
...
.defines(function() {
    ...
    ZoneServer = ig.Game.extend({
        ...
        gatewaySockets: {},
        frame: 0,
        snapshots_per_second: 4,
        snapshot_timer: new ig.Timer(),
